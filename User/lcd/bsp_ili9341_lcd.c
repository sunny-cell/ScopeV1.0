...
void ILI9341_DrawDottedLine_ ( uint16_t usX1, uint16_t usY1, uint16_t usX2, uint16_t usY2 )
{
	uint16_t us; 
	uint16_t usX_Current, usY_Current;
	...
	else 
  {
	...
	for ( us = 0; us <= lDistance + 1; us =us+4 )//画线输出 
	{  
		ILI9341_SetPointPixel ( usX_Current, usY_Current );//画点 
		...
		{ 
			lError_X -= lDistance; 
			usX_Current += lIncrease_X+4; 
		}  
		
		if ( lError_Y > lDistance ) 
		{ 
			lError_Y -= lDistance; 
			usY_Current += lIncrease_Y+4; 
		} 
